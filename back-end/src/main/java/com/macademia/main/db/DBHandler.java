...
public class DBHandler {
	...
	public DBHandler(String Host, String Database, String Username, String Password, Boolean Overwrite) throws SQLException {
		...
		// and Sections upon instantiation.
	}
	...
	/**
	 * Gets a list of all students in the system using the exact same method as
	 * getDepartments()
	 * 
	 * @return
	 * @throws SQLException
	...
	public List<Student> getStudents() throws SQLException {
		...
		// I know we access the database like twice to get this data but this allows the
		// handler to kick in. Otherwise this'd be a mess.
		while (RS.next()) {
			Students.add(getStudent(RS.getString("ID")));
		}
		RS.close();
		return Students;
	}
	...
	public Student getStudent(User user) throws SQLException {
		...
		for (String MatID : RS.getString("Matriculas").split(",")) {
			if (!MatID.isBlank()) {
				...
				} // Make sure we actually found the matricula.
			}
		}
		...
		for (String PriorityCourse : RS.getString("PriorityCourses").split(",")) {
			if (!PriorityCourse.isBlank()) {
				...
				if (pcourse != null) {
					ReturnStudent.addPriority(pcourse);
				} // make sure we actually found the course.
			}
		}
		...
		for (String CourseTaken : RS.getString("CoursesTaken").split(",")) {
			if (!CourseTaken.isBlank()) {
				...
				if (tcourse != null) {
					ReturnStudent.addCourseTaken(tcourse);
				} // make sure we actually found the course.
			}
		}
	...
	/**
	 * Gets a list of all departments in the database. If they have not been loaded,
	 * they will be loaded.
	 * 
	 * @return
	 * @throws SQLException
	...
	public List<Department> getDepartments() throws SQLException {
		...
		// handler to kick in. Otherwise this'd be a mess.
		while (RS.next()) {
			Departments.add(getDepartment(RS.getString("ID")));
		}
		RS.close();
		return Departments;
	}
	...
	public Department getDepartment(String ShortNameID) throws SQLException {
		...
		if (DepartmentMap.containsKey(ShortNameID)) {
			return DepartmentMap.get(ShortNameID);
		}
	...
	public Course getCourse(String Course) throws SQLException {
		...
		if (dep == null) {
			return null;
		} // If the department doesn't exist, then obviously neither does the course.
		if (dep.getCatalog().containsKey(CourseID)) {
			return dep.getCatalog().get(CourseID);
		}
		...
		if (RS.getBoolean("L") != RequestedL) {
			if (!RS.next()) {
				RS.close();
				return null;
			}
		} // If L doesn't match, RS.NEXT. If Not RS.NEXT, RS.CLOSE.
		...
		for (String prereq : Prereq) {
			if (prereq.length() != 0) {
				...
				if (preq != null) {
					TheCourse.addPrereq(preq);
				} // make sure we found the prereq
	...
	public List<Section> getAllSections() throws SQLException {
		...
		while (RS.next()) {
			Sections.add(getSection(RS.getString("ID")));
		}
		RS.close();
		return Sections;
	}
	...
	public boolean StudentExists(String StudentNumber) throws SQLException {
		return getStudent(StudentNumber) != null;
	}

	public boolean MatriculaExists(int ID) throws SQLException {
		return getMatricula(ID) != null;
	}

	public boolean DepartmentExists(String ShortNameID) throws SQLException {
		return getDepartment(ShortNameID) != null;
	}

	public boolean CourseExists(String Course) throws SQLException {
		return getCourse(Course) != null;
	}

	public boolean SectionExists(String SectionID) throws SQLException {
		return getSection(SectionID) != null;
	}
	...
	/**
	 * Does what it says: Saves everything \n\n Saves every department, and within
	 * it saves every course, and within it saves every section.
	 */
	...
	public void SaveUser(User user) throws SQLException {
		if (UserExists(user.getUsername())) {
			UpdateUsers(user.getUsername(), user.getPassword());
		} else {
			InsertIntoUsers(user.getUsername(), user.getPassword());
		}
	}
	...
	public void SaveStudent(Student stud) throws SQLException {
		...
		if (!UserExists(stud.getUsername())) {
			// The tiedUser is not in the system, but we can actually just *a d d i t o u r
			// s e l v e s * :woah:
		...
		// Save the Matriculas
		for (Matricula Mat : stud.getMatriculas()) {
			SaveMatricula(Mat);
		}
	...
	public int SaveMatricula(Matricula Mat) throws SQLException {
		...
		if (SemestersBetweenToday(Mat.getPeriod()) > 0) {
			throw new IllegalArgumentException("This Matricula is too old to save");
		}
		...
		if (Mat.getID() == -1) {
			...
			do {
				NewID = rand.nextInt();
			} while (MatriculaExists(NewID)); // Generate a new ID while there exists a matricula with that ID
	...
	/**
	 * Saves the specified department to the SQL Database, including saving every
	 * course within, and every section within those courses.
	 * 
	 * @param dep
	 */
	public void SaveDepartment(Department dep) throws SQLException {
		// Save the department itself
		if (DepartmentExists(dep.getShortName())) {
			UpdateDepartments(dep.getShortName(), dep.getName(), dep.getColor());
		} else {
			InsertIntoDepartments(dep.getShortName(), dep.getName(), dep.getColor());
		}
		...
		// Save the course
		for (Course course : dep.getCatalog().values()) {
			SaveCourse(course);
		}
	}

	/**
	 * Saves the specified course to the SQL Database, including saving every
	 * department within
	 * 
	 * @param course
	 */
	public void SaveCourse(Course course) throws SQLException {
		...
		if (!DepartmentExists(course.getDept())) {
			throw new IllegalArgumentException(
					"Department is not in the database! Register it *BEFORE* adding this course");
		}
	...
	public void SaveSeciton(Section sect) throws SQLException {
		...
		if (HeadCourse == null) {
			throw new IllegalArgumentException(
					"Course is not in the database! Register it *BEFORE* adding this course");
		}
		...
		if (SectionExists(SectionID)) {
			UpdateSections(SectionID, L, sect.getDay(), Time, Location, Prof, CurCap, MaxCap);
		} else {
			InsertIntoSections(SectionID, L, sect.getDay(), Time, Location, Prof, CurCap, MaxCap);
		}
	...
	/**
	 * Removes the department from the department map, deletes it from the database,
	 * and deletes all courses in its course catalog.
	 * 
	 * @param dep
	 * @throws SQLException
	 */
	public void deleteDepartment(Department dep) throws SQLException {
		if (DepartmentMap.containsKey(dep.getShortName())) {
			DepartmentMap.remove(dep.getShortName());
		} // if it's in the map, remove it from the map.
		...
		// And that's it... at least until we add matriculas as course programs... but I
	...
	/**
	 * Handles getting a result set from a given table where the column matches the
	 * value
	 * 
	 * @return SELECT * FROM (TABLE) WHERE (COLUMN) = '(VALUE)';
	 * @throws SQLException
	 */
	private ResultSet GetFromWhere(String Table, String Column, String Value) throws SQLException {
		return SQLConn.createStatement()
				.executeQuery("SELECT * FROM " + Table + " WHERE " + Column + " = '" + Value + "';");
	}

	/**
	 * Handles getting a result from a given table, where the column is *like* the
	 * value
	 * 
	 * @return SELECT * FROM (TABLE) WHERE (COLUM) LIKE '(VALUE)'
	 * @throws SQLException
	 */
	private ResultSet GetFromWhereLike(String Table, String Column, String Value) throws SQLException {
		return SQLConn.createStatement()
				.executeQuery("SELECT * FROM " + Table + " WHERE " + Column + " LIKE '" + Value + "';");
	}
	 ...
	 * @throws SQLException
	 */
	private ResultSet GetEverythingFrom(String Table) throws SQLException {
		return SQLConn.createStatement().executeQuery("SELECT * FROM " + Table + ";");
	}

	private ResultSet selectUser(String Username) throws SQLException {
		return GetFromWhere("Users", "Username", Username);
	}

	private ResultSet selectStudents() throws SQLException {
		return GetEverythingFrom("Students");
	}

	private ResultSet selectStudentFromUsername(String Username) throws SQLException {
		return GetFromWhere("Students", "TiedUser", Username);
	}

	private ResultSet selectStudentFromID(String StudentID) throws SQLException {
		return GetFromWhere("Students", "ID", StudentID);
	}

	private ResultSet selectMatricula(int ID) throws SQLException {
		return GetFromWhere("Matriculas", "ID", ID + "");
	}

	private ResultSet selectDepartments() throws SQLException {
		return GetEverythingFrom("Departments");
	}

	private ResultSet selectDepartment(String ShortName) throws SQLException {
		return GetFromWhere("Departments", "ID", ShortName);
	}

	private ResultSet selectAllCourses() throws SQLException {
		return GetEverythingFrom("Courses");
	}

	private ResultSet selectCourses(String Department) throws SQLException {
		return GetFromWhereLike("Courses", "ID", Department + "%");
	}

	private ResultSet selectCourse(String ID) throws SQLException {
		return GetFromWhere("Courses", "ID", ID.substring(0, 8));
	}

	private ResultSet selectAllSections() throws SQLException {
		return GetEverythingFrom("Sections");
	}

	private ResultSet selectSections(String Course) throws SQLException {
		return GetFromWhereLike("Sections", "ID", Course + "%");
	}

	private ResultSet selectSection(String ID) throws SQLException {
		return GetFromWhere("Sections", "ID", ID);
	}
	 ...
	 * @throws SQLException
	 */
	private int deleteFromWhere(String Table, String Column, String Value) throws SQLException {
		return SQLConn.createStatement()
	...
	private int deleteStudent(String StudentID) throws SQLException {
		return deleteFromWhere("Students", "ID", StudentID);
	}

	private int deleteMatricula(int ID) throws SQLException {
		return deleteFromWhere("Matriculas", "ID", ID + "");
	}

	private int deleteDepartment(String ShortName) throws SQLException {
		return deleteFromWhere("Departments", "ID", ShortName);
	}

	private int deleteCourse(String ID) throws SQLException {
		return deleteFromWhere("Courses", "ID", ID.substring(0, 8));
	}

	private int deleteSection(String ID) throws SQLException {
		return deleteFromWhere("Sections", "ID", ID);
	}
	 ...
	 * @param PriorityCourses Comma separated list of Priority Courses (IE:
	 *                        DRAM3001, DRAM3002, DRAM3003)
	 * @throws SQLException
	 */
	private void InsertIntoStudents(String ID, String Name, String TiedUsername, String Department, String Matriculas,
			String PriorityCourses, String CoursesTaken, String Turn) throws SQLException {
	 ...
	 * @param Sections Comma separated list of Section IDs (IE: DRAM3001-020,
	 *                 DRAM3002-020)
	 * @param Period   Period of this matricula (One of the following: SPRING,
	 *                 SUMMER1, SUMMER2, EXT_SUMMER, FALL)
	 * @param Year     Year of this Matricula
	 * @throws SQLException
	 */
	private void InsertIntoMatriculas(int ID, String Sections, String Period, int Year, boolean ReadOnly)
			throws SQLException {
	 ...
	 * @param Name      Name of this department (IE: Department of Software
	 *                  Engineering)
	 * @throws SQLException
	 */
	 ...
	 * @param Prereqs Comma separated list of Course IDs that are Prerequesites (IE:
	 *                DRAM3000, ENGL3001)
	 * @param Coreqs  Comma separated list of Course IDs that are Corequesites (IE:
	 *                DRAM3003, ENGL3003)
	 * @throws SQLException
	 */
	 ...
	 * @param Time     Times of this course in MILITARY TIME. Split by a DASH (IE:
	 *                 8:30-9:30)
	 ...
	 * @throws SQLException
	 */
	private void InsertIntoSections(String ID, boolean L, String Days, String Time, String Location, String Prof,
			int CurCap, int MaxCap) throws SQLException {
	 ...
	 * @throws SQLException
	 */
	private void UpdateStudents(String ID, String Name, String TiedUsername, String Department, String Matriculas,
			String PriorityCourses, String CoursesTaken, String Turn) throws SQLException {
	 ...
	 * @throws SQLException
	 */
	private void UpdateMatriculas(int ID, String Sections, String Period, int Year, boolean ReadOnly)
			throws SQLException {
	 ...
	 * @throws SQLException
	 */
	private void UpdateSections(String ID, boolean L, String Days, String Time, String Location, String Prof,
			int CurCap, int MaxCap) throws SQLException {
	...
	public static String ListOfCoursesToString(List<Course> Courses) {
		...
		if (ListAsString.length() > 0) {
			ListAsString = ListAsString.substring(1);
		} // Handles the first comma
		return ListAsString;
	}
	 ...
	 *         since the provided period. <br>
	 *         Returns a negative number if the semester is in the future. Returns 0
	 *         if the semester is the current one.
	 */
	public static int SemestersBetweenToday(MatriculaPeriod period) {
		...
		// Now, time to count.
		return ((date.getYear() * 3) + CurrentSemester) - ((period.getMatyear() * 3) + PastSemester); // Years between
																										// times 3
																										// because three
																										// semester per
																										// year.
		// -1 because if not this would assume 3 between a matricula for winter one year
		// and spring the next.

	}

	/**
	 * Utility to turn a list of matriculas into a saved list of comma separated
	 * Matricula IDs
	 * 
	 * @param collection
	 * @return
