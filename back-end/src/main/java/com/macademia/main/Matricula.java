...
public class Matricula implements Comparable<Matricula> {
	...
	// had to include both section and course whenever adding a new section into a
	// matricula
	...
	public void removeCourse(Course f) {
		...
		if (e == null) {
			// We didn't find it, but remove the course anyway. It should be removed in that
			// case.
			courses.remove(f);
			return;
		}
	 ...
	 * day, and contains sections that occur on those days.
	 * 
	 * @return
	 */
	public List<List<Section>> getSectionsByDay() {
		...
		for (int Day = 0; Day < 7; Day++) {
			Week.add(new ArrayList<Section>());
		} // Make sure each de-esta cosa has an arraylist.

		// now go through each section.
		for (Section section : sections) {
			for (char DayLetter : section.getDay().toCharArray()) { // Now go through each letter in their days
				Week.get(LetterToDay(DayLetter)).add(section); // And add them to one of the 6 de-estas cosas using
																// letter to day.
			}
		}
	 ...
	 */
	private static int LetterToDay(char Letter) {
		return "LMWJVSD".indexOf(Letter);
	}
	...
	public int compareTo(Matricula m) {
		if (this.period.getMatyear() == m.getPeriod().getMatyear())
			return this.semesterValue(m.getPeriod().getSemesterAsString())
					- this.semesterValue(this.period.getSemesterAsString());
		else
			return m.getPeriod().getMatyear() - this.period.getMatyear();
	}

	public int semesterValue(String s) {
		switch (s) {
			case "SPRING":
				return 0;
			case "SUMMER":
