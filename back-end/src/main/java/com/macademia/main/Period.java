...
/**
 * Holds two integers representing two times for sections using ints to store
 * military time.<br>
 * <br>
 * IE, if a period is from 3:15 PM - 3:30 PM, it's stored as START 1515, END
 * 1530.<br>
 * <br>
...
public class Period {
    ...
    public Period(int start, int end) {
        ...
        setEnd(end);
        if (end < start) {
            throw new IllegalArgumentException("Period cannot start after it ends, or end before it starts");
    ...
    public static String intToStandardTime(int Time) {
        ...
        if (Time >= 1200) {
            PM = true;
        } // If it's greater than 1200, we're in the PM
        if (Time >= 1300) {
            Time -= 1200;
        } // If it's greater than 1300 (IE 1:00 PM), subtract 1200

        String TimeString = intToMilitaryTime(Time); // Use IntToMilitaryTime to convert it into a time

        if (PM) {
            return TimeString + " PM";
        } // Append AM or PM accordingly
        else {
     ...
     * @return TRUE if the other period starts or ends between this period's start
     *         or end.
     */
    public boolean Conflict(Period per) {
        return (this.start < per.start && per.start < this.end) || (this.start < per.end && per.end < this.end);
    }
    ...
    public String toStandardTimeString() {
        return intToStandardTime(start) + "-" + intToStandardTime(end);
    }
    ...
    public String toMilitaryTimeString() {
        return intToMilitaryTime(start) + "-" + intToMilitaryTime(end);
    }
